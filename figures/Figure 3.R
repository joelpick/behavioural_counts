...
setEPS()
pdf("figures/PR_fig3.pdf", height=9, width=15)

layout(matrix(c(1,1,2,2,3,3,4,5,5,6,6,7),nrow=2,byrow=TRUE))

par(mar=c(3,7,3,1), cex.lab=1.75, cex.axis=1.2,oma = c(3, 0, 0, 0))


plot(sqrt(means)/means ~ means , type="l", ylab=TeX("$CV = \\frac{\\sigma_x}{\\bar{x}}$"), xlab="", ylim=c(0,1), lwd=2)
...
plot(NA, ylim=c(0,1), xlim=c(0,100), ylab="CV", xlab="")
...
mtext('E',side=3, line=0.5, cex=1.5, adj=0)


mtext("Mean number of observed arrivals",line=0.5,side=1,outer=TRUE, cex=1.75)
