...
\node[align=left, scale=1.5] at (0.8\textwidth,27) {\textbf {Example Population} \\ Provisioning Rate = \rate\ visits/hour,
...
% top text
\node[anchor= west, align=left, scale=1.5] at (0,\y+6) {\textbf{Study \studies[\i]} - Observation period = \pgfmathparse{(\time*60)} \pgfmathprintnumber[assume math mode=true]{\pgfmathresult}\ mins, Mean number of observed visits = \pgfmathparse{(\time*\rate)} \pgfmathprintnumber[assume math mode=true]{\pgfmathresult}};%
...
\node[align=right, anchor= east, scale=1.5] at (0,\y+2.75) {Assuming No \\ Poisson Error} ;
...
\node[align=right, anchor= east, scale=1.5] at (0,\y+0.75) {Correcting for \\ Poisson Error} ;
...
\node[anchor=south east, scale=1.5] at (\legx,\legy-3) {Poisson distributed error} ;
